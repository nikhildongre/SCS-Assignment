...
public class RetriveFileService {
	...
	public List<FileLockedByUser> getFiles(String path){
		
		List<FileDocument> files=null;
		List<FileLockedByUser> lockedFileList = new ArrayList<FileLockedByUser>();
		FileLockedByUser fileLockedByUser=null;
		
		 try {
			files=filedao.findByPathClone("^"+path);
					
			if(files==null || files.isEmpty()){
				System.out.println("Invalid URL/Url is not accessible for this user");
			}else{
				for (FileDocument file : files) {
					if(file.isFile() && file.isLocked()){
						fileLockedByUser=new FileLockedByUser(file.getId(), file.getName(), file.getLockedBy(), file.isLocked());
						lockedFileList.add(fileLockedByUser);
					}
				}
			}
		} catch (Exception e1) {
